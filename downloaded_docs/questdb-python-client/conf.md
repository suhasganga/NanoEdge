...
When constructing a [sender](sender.html#sender) you can pass a configuration string
to the [`Sender.from_conf`](api.html#questdb.ingress.Sender.from_conf "questdb.ingress.Sender.from_conf") method.
...
If you’re unsure which protocol to use, see [ILP/TCP or ILP/HTTP](sender.html#sender-which-protocol).
...
users in the [Enterprise quickstart](https://questdb.com/docs/guides/enterprise-quick-start/#4-ingest-data-influxdb-line-protocol)
and the [role-based access control](https://questdb.com/docs/operations/rbac/) guides.

### HTTP Bearer Token[¶](#http-bearer-token "Link to this heading")

* `token` - `str`: Bearer token for HTTP authentication.

### HTTP Basic Auth[¶](#http-basic-auth "Link to this heading")

* `username` - `str`: Username for HTTP basic authentication.
* `password` - `str`: Password for HTTP basic authentication.

### TCP Auth[¶](#tcp-auth "Link to this heading")
...
See the [TCP Authentication and TLS](examples.html#auth-and-tls-example) example for more details.

## TLS[¶](#tls "Link to this heading")

TLS in enabled by selecting the `tcps` or `https` protocol.

See the [QuestDB enterprise TLS documentation](https://questdb.com/docs/operations/tls/)
...
* `tls_ca` - The remote server’s certificate authority verification mechamism.
  ...
  + `'webpki_roots'`: Use the
    [webpki-roots](https://crates.io/crates/webpki-roots) Rust crate to
    recognize certificates.
  + `'os_roots'`: Use the OS-provided certificate store.
  + `'webpki_and_os_roots'`: Use both the
    [webpki-roots](https://crates.io/crates/webpki-roots) Rust crate and
...
The following parameters control the [Auto-flushing](sender.html#sender-auto-flush) behavior.
...
### `auto_flush_interval`[¶](#auto-flush-interval "Link to this heading")
...
[`Sender.row`](api.html#questdb.ingress.Sender.row "questdb.ingress.Sender.row") or the
[`Sender.dataframe`](api.html#questdb.ingress.Sender.dataframe "questdb.ingress.Sender.dataframe") methods.
...
The [Advanced Usage](sender.html#sender-advanced) documentation should help you.
...
## Buffer[¶](#buffer "Link to this heading")
...
## HTTP Request[¶](#http-request "Link to this heading")
