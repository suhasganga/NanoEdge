...
:func:`flush ` often enough.
You might be having issues with the :class:`Sender `'s
:ref:`auto-flush ` feature.
...
\* For TCP/TCPS: You must explicitly configure ``protocol\_version=3`` in your
configuration string::
tcp::addr=localhost:9009;protocol\_version=3;
...
.. code-block:: python
import textwrap
with Sender.from\_conf(...) as sender:
# sender.row(...)
# sender.row(...)
# ...
pending = str(sender)
...
Asking for help
===============
The best way to get help is through our `Community Forum `\_.