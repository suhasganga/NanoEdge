...
If you haven't already, please read the [Introduction](/lightweight-charts/tutorials/customization/intro). At the bottom of that page, you will find a starting file containing everything you need to get up and running with this tutorial (including some sample data to display on the chart).

Our first task will be to create a chart and get it visible on the page. A more detailed explanation of these steps is available [here](/lightweight-charts/docs).

## Adding the Lightweight Charts™ script[​](#adding-the-lightweight-charts-script "Direct link to Adding the Lightweight Charts™ script")
...
We can create the chart instance by calling the [`createChart`](/lightweight-charts/docs/api/functions/createChart) method on the `LightweightCharts` global variable, and providing an HTML div element as the first parameter to the method. The `createChart` method can take a second parameter for options which will be explored and used later in the guide.
...
The `createChart` method returns an [IChartApi](/lightweight-charts/docs/api/interfaces/IChartApi) instance which will be our main point of contact for interacting with the API.

## Creating some sample data[​](#creating-some-sample-data "Direct link to Creating some sample data")
...
## Adding a candlestick series[​](#adding-a-candlestick-series "Direct link to Adding a candlestick series")

We can now add a [candlestick series](/lightweight-charts/docs/series-types#candlestick) to our chart by using the `addCandlestickSeries` method on the `chart` variable (which is an [IChartApi](/lightweight-charts/docs/api/interfaces/IChartApi) instance). This will return a series instance which we will assign to a variable named `mainSeries`.

`mainSeries` will be an instance of [`ISeriesApi`](/lightweight-charts/docs/api/interfaces/ISeriesApi) which will provide the API for interacting with the series and it's options.
...
## Automatically resizing the chart when the window is resized (Optional)[​](#automatically-resizing-the-chart-when-the-window-is-resized-optional "Direct link to Automatically resizing the chart when the window is resized (Optional)")
...
[View in a new window](/lightweight-charts/fdd4d73df0fa987528278b79ea5f92a8.html)
...
You can download the HTML file for example at this stage [here](/lightweight-charts/fdd4d73df0fa987528278b79ea5f92a8.html) in case you've encountered a problem or would like to start the next step from this point.
