...
It is recommend that you follow the guide from the [start](/lightweight-charts/tutorials/customization/intro). However, if you are only interested in the content on this page then take a look at the [full code](#complete-code) at the bottom of page for context of how this code fits into a working example.
...
## Setting the background color of the HTML body[​](#setting-the-background-color-of-the-html-body "Direct link to Setting the background color of the HTML body")
...
## Applying options[​](#applying-options "Direct link to Applying options")
...
* During Creation* Using applyOptions
...
## Adjusting the background and text colors for the chart[​](#adjusting-the-background-and-text-colors-for-the-chart "Direct link to Adjusting the background and text colors for the chart")

We can adjust the chart's background color and text color by specifying our desired colors within the options for [IChartApi](/lightweight-charts/docs/api/interfaces/IChartApi). A full list of the available options for the chart instance can be viewed here: [ChartOptionsBase](/lightweight-charts/docs/api/interfaces/ChartOptionsBase).
...
## (Optional) Setting a desired height and width for the chart[​](#optional-setting-a-desired-height-and-width-for-the-chart "Direct link to (Optional) Setting a desired height and width for the chart")
...
## Adjusting the border colors for the axes[​](#adjusting-the-border-colors-for-the-axes "Direct link to Adjusting the border colors for the axes")

We can set the border colors for the price and time scales by using the `applyOptions` method on the [IPriceScale](/lightweight-charts/docs/api/interfaces/IPriceScaleApi) and [ITimeScale](/lightweight-charts/docs/api/interfaces/ITimeScaleApi) instances. We can get references to these instances by using the `priceScale()` and `timeScale()` methods on a chart instance.
...
[View in a new window](/lightweight-charts/14e2052f6e317b7e5e74a019ebc64bc8.html)
...
You can download the HTML file for example at this stage [here](/lightweight-charts/14e2052f6e317b7e5e74a019ebc64bc8.html) in case you've encountered a problem or would like to start the next step from this point.
