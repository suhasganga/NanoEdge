...
To create an overlay price scale, assign [`priceScaleId`](/lightweight-charts/docs/next/api/interfaces/SeriesOptionsCommon#pricescaleid) to a series.
...
Further, you can use the provided price scale ID to retrieve its API object using the [`IChartApi.priceScale`](/lightweight-charts/docs/next/api/interfaces/IChartApi#pricescale) method.
...
To modify the left price scale, use the [`leftPriceScale`](/lightweight-charts/docs/next/api/interfaces/ChartOptionsBase#leftpricescale) option.
For the right price scale, use [`rightPriceScale`](/lightweight-charts/docs/next/api/interfaces/ChartOptionsBase#rightpricescale).
To change the default settings for an overlay price scale, use the [`overlayPriceScales`](/lightweight-charts/docs/next/api/interfaces/ChartOptionsBase#overlaypricescales) option.

You can use the [`IChartApi.priceScale`](/lightweight-charts/docs/next/api/interfaces/IChartApi#pricescale) method to retrieve the API object for any price scale.
Similarly, to access the API object for the price scale that a series is attached to, use the [`ISeriesApi.priceScale`](/lightweight-charts/docs/next/api/interfaces/ISeriesApi#pricescale) method.

## Remove price scale[​](#remove-price-scale "Direct link to Remove price scale")

The default left and right price scales cannot be removed, you can only hide them by setting the [`visible`](/lightweight-charts/docs/next/api/interfaces/PriceScaleOptions#visible) option to `false`.
