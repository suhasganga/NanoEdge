...
* Added an Image Watermark Primitive: In addition to the usual text based watermark, there is now
  an image watermark feature provided by the [`createImageWatermark`](/lightweight-charts/docs/next/api/functions/createImageWatermark) plugin.
...
* **ESM builds**: Import [`createTextWatermark`](/lightweight-charts/docs/next/api/functions/createTextWatermark) directly.
