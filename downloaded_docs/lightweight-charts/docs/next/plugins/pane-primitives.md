...
Pane Primitives can be added to a chart using the `attachPrimitive` method on the [`IPaneApi`](/lightweight-charts/docs/next/api/interfaces/IPaneApi) interface. Here's an example:
...
To create a Pane Primitive, you should implement the [`IPanePrimitive`](/lightweight-charts/docs/next/api/type-aliases/IPanePrimitive) interface. This interface is similar to [`ISeriesPrimitive`](/lightweight-charts/docs/next/api/type-aliases/ISeriesPrimitive), but with some key differences:
...
For more details on implementing Pane Primitives, refer to the [`IPanePrimitive`](/lightweight-charts/docs/next/api/type-aliases/IPanePrimitive) interface documentation.