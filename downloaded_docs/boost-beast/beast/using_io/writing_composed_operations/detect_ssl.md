#### [Detect SSL 💡](detect_ssl.html "Detect SSL 💡")

In this example we will build a simple function to detect the presence
of the [TLS
client handshake](https://tools.ietf.org/html/rfc2246#section-7.4) given an input buffer sequence. Then we build
...
of [*ConstBufferSequence*](../../../../../../../doc/html/boost_asio/reference/ConstBufferSequence.html).
...
Our goal is to create a *stream algorithm*: a function
...
is called an *initiating function*. While the synchronous
...
of the initiation function object. They are placed in the `detail` namespace since they should not
be public:
...
object. Depending on the specialization of `async_result`
for the type of `CompletionToken`,
...
the signature `void(error_code,
std::size_t)`
which is exactly the signature needed when performing asynchronous reads.
This function is a transformation of the synchronous version of `detect_ssl` above, but with the inversion
...
The examples [advanced-server](../../../../../example/advanced/server/advanced_server.cpp) and
[advanced-server-flex](../../../../../example/advanced/server-flex/advanced_server_flex.cpp)
use
