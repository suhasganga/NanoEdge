### [HTTP Comparison to Other Libraries](http_comparison_to_other_librari.html "HTTP Comparison to Other Libraries")
...
##### [cpp-netlib](http_comparison_to_other_librari.html#beast.design_choices.http_comparison_to_other_librari.cpp_netlib)

[**cpp-netlib**](https://github.com/cpp-netlib/cpp-netlib/tree/092cd570fb179d029d1865aade9f25aae90d97b9) is a network programming library
...
a goal. The library is based on Boost.Asio and bills itself as *"a
collection of network related routines/implementations geared towards providing
a robust cross-platform networking library"*. It cites *"Common
Message Type"* as a feature. As of the branch previous linked,
...
* There is no way to defer the commitment of the type for `body_` to after the headers are read
  in.
...
* The use of `string_type`
  (a customization point) for source, destination, and body suggests that
  `string_type` models a
  **ForwardRange** whose `value_type`
  is `char`. This representation
  is less than ideal, considering that the library is built on Boost.Asio.
  Adapting a [*DynamicBuffer*](../concepts/DynamicBuffer.html "DynamicBuffer")
  to the required forward range destroys information conveyed by the [*ConstBufferSequence*](../../../../../../doc/html/boost_asio/reference/ConstBufferSequence.html)
  ...
  will be less efficient than an equivalent [Networking
  TS](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2018/n4771.pdf) conforming implementation.
* The library uses specializations of `string<Tag>` to change the type of string used
  ...
  library's use of the string trait is limited to selecting between `std::string` and `std::wstring`.
...
* The specialized trait classes generate a proliferation of small additional
  ...
  and intrude into the `boost::network::http`
...
##### [Boost.HTTP](http_comparison_to_other_librari.html#beast.design_choices.http_comparison_to_other_librari.boost_http)

[**boost.http**](https://github.com/BoostGSoC14/boost.http/tree/45fc1aa828a9e3810b8d87e669b7f60ec100bff4) is a library resulting from the
...
* This container cannot model a complete message. The *start-line*
  ...
  programs](https://github.com/BoostGSoC14/boost.http/blob/45fc1aa828a9e3810b8d87e669b7f60ec100bff4/example/basic_router.cpp#L81). The `500`
  and `"OK"` arguments
  ...
  ```
* `headers_`, `body_`, and `trailers_`
  may only be default-constructed, since there are no explicitly declared
  constructors.
* There is no way to defer the commitment of the **Body**
...
* The trailers are stored in a separate object. Aside from the combinatorial
  ...
  operate on messages.
* The declarations imply that `std::vector`
  ...
  that the library is built on Boost.Asio. Adapting a [*DynamicBuffer*](../concepts/DynamicBuffer.html "DynamicBuffer")
  to the required forward range destroys information conveyed by the [*ConstBufferSequence*](../../../../../../doc/html/boost_asio/reference/ConstBufferSequence.html)
  and [*MutableBufferSequence*](../../../../../../doc/html/boost_asio/reference/MutableBufferSequence.html)
  ...
  [Networking
  TS](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2018/n4771.pdf) conforming implementation.
* The **Body** customization point constrains
...
##### [C++ REST SDK (cpprestsdk)](http_comparison_to_other_librari.html#beast.design_choices.http_comparison_to_other_librari.c_rest_sdk_cpprestsdk)

[**cpprestsdk**](https://github.com/Microsoft/cpprestsdk/tree/381f5aa92d0dfb59e37c0c47b4d3771d8024e09a) is a Microsoft project which *"...aims
to help C++ developers connect to and interact with services"*.
...
uses the asynchronous model defined by Microsoft's [**Concurrency Runtime**](https://msdn.microsoft.com/en-us/library/dd504870.aspx). Identifiers from the
[**`pplx` namespace**](https://msdn.microsoft.com/en-us/library/jj987780.aspx)
define common asynchronous patterns such as tasks and events. The `concurrency::streams::istream` parameter and `m_data_available`
...
it does with actually modeling the HTTP message as described in [rfc7230](https://tools.ietf.org/html/rfc7230):
...
* There are no compile-time customization points at all. The only customization
  is in the `concurrency::streams::istream`
  and `concurrency::streams::ostream` reference parameters. Presumably,
...
[**A Universal Model for Asynchronous Operations**](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2013/n3747.pdf)
(Kohlhoff).