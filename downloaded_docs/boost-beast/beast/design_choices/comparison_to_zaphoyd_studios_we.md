### [Comparison to Zaphoyd Studios WebSocket++](comparison_to_zaphoyd_studios_we.html "Comparison to Zaphoyd Studios WebSocket++")

How does this compare to [websocketpp](https://www.zaphoyd.com/websocketpp), an alternate header-only WebSocket implementation?
:   1. Synchronous Interface
    :   Beast offers full support for WebSockets using a synchronous
        ...
        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp#L774) | websocketpp |
        | --- | --- |
        | ```table-programlisting template<class DynamicBuffer> void read(DynamicBuffer& dynabuf) ``` | *<not available>* |

    2. Connection Model
    :   websocketpp supports multiple transports by utilizing a trait,
        the `config::transport_type` ([asio
        transport example](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/transport/asio/connection.hpp#L60)) To get an idea of the complexity involved
        with implementing a transport, compare the asio transport to
        the [`iostream` transport](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/transport/iostream/connection.hpp#L59)
        ...
        **`NextLayer`**
        template argument The type requirements for **`NextLayer`** are already
        familiar to users as they are documented in Asio: [*AsyncReadStream*](../../../../../../doc/html/boost_asio/reference/AsyncReadStream.html),
        ...
        2). Note that websocketpp connections are passed by `shared_ptr`. Beast does not
        use `shared_ptr`
        anywhere in its public interface. A `beast::websocket::stream`
        is constructible and movable in a manner identical to a `boost::asio::ip::tcp::socket`. Callers can put such
        objects in a `shared_ptr`
        if they want to, but there is no requirement to do so.

        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp) | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/connection.hpp#L234) |
        | --- | --- |
        | ```table-programlisting template<class NextLayer> class stream {     NextLayer next_layer_;     ... } ``` | ```table-programlisting template <typename config> class connection : public config::transport_type::transport_con_type , public config::connection_base { public:     typedef lib::shared_ptr<type> ptr;     ... } ``` |
    ...
    :   websocketpp provides multi-role support through a hierarchy of
        ...
        | Beast | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/roles/server_endpoint.hpp#L39), [also](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/roles/client_endpoint.hpp#L42) |
        | --- | --- |
        | *<not needed>* | ```table-programlisting template <typename config> class client : public endpoint<connection<config>,config>; template <typename config> class server : public endpoint<connection<config>,config>; ``` |
    ...
    :   websocketpp uses mutexes to protect shared data from concurrent
        ...
        handler, through the associated executor mechanism.
        ...
        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/impl/read_frame_op.ipp#L118) | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/transport/iostream/connection.hpp#L706) |
        | --- | --- |
        | ```table-programlisting mutex_type m_read_mutex; ``` |
    ...
    :   websocketpp requires a one-time call to set the handler for each
        ...
        associated with the handler through the use of argument dependent
        lookup overloads of functions such as `bind_allocaotr`.
        ...
        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp#L834) | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/connection.hpp#L281), [also](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/connection.hpp#L473) |
        | --- | --- |
        | ```table-programlisting template<     class DynamicBuffer,    // Supports user defined types     class ReadHandler       // Handler is NOT type-erased > typename async_completion<  // Return value customization     ReadHandler,            // supports futures and coroutines     void(error_code)         >::result_type async_read(     DynamicBuffer& dynabuf,     ReadHandler&& handler); ``` | ```table-programlisting typedef lib::function<     void(connection_hdl,message_ptr)         > message_handler; void set_message_handler(message_handler h); ``` |

    6. Extensible Asynchronous Model
    :   Beast fully supports the [Extensible
        Asynchronous Model](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2014/n3896.pdf) developed by Christopher Kohlhoff,
        author of Boost.Asio (see Section 8).

        Beast websocket asynchronous interfaces may be used seamlessly
        with `std::future` stackful/stackless coroutines,
        or user defined customizations.

        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/impl/stream.ipp#L378) | websocketpp |
        | --- | --- |
        | ```table-programlisting beast::async_completion<     ReadHandler,     void(error_code)> completion{handler}; read_op<     DynamicBuffer, decltype(completion.handler)>{         completion.handler, *this, op, buffer};  return completion.result.get();     // Customization point ``` | *<not available>* |
    ...
    :   websocketpp defines a message buffer, passed in arguments by
        ...
        uses a `std::string` to hold the payload.
        If an incoming message is broken up into multiple frames, the
        string may be reallocated for each continuation frame. The `std::string` always uses the standard
        ...
        of [*DynamicBuffer*](../concepts/DynamicBuffer.html "DynamicBuffer")
        (modeled after `boost::asio::streambuf`).

        Beast comes with the class [`basic_multi_buffer`](../ref/boost__beast__basic_multi_buffer.html "basic_multi_buffer"),
        an efficient implementation of the [*DynamicBuffer*](../concepts/DynamicBuffer.html "DynamicBuffer")
        ...
        particular memory management model on callers. The [`basic_multi_buffer`](../ref/boost__beast__basic_multi_buffer.html "basic_multi_buffer") provided
        by beast supports standard allocators through a template argument.
        Use the [*DynamicBuffer*](../concepts/DynamicBuffer.html "DynamicBuffer")
        that comes with beast, customize the allocator if you desire,
        or provide your own type that meets the requirements.

        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp#L774) | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/message_buffer/message.hpp#L78) |
        | --- | --- |
        | ```table-programlisting template<class DynamicBuffer> read(DynamicBuffer& dynabuf); ``` | ```table-programlisting template <template<class> class con_msg_manager> class message { public:     typedef lib::shared_ptr<message> ptr;     ...     std::string m_payload;     ... }; ``` |
    ...
    :   When sending a message, websocketpp requires that the payload
        ...
        object meeting the requirements of \_\_ConstBufferSequence, permitting
        ...
        data to many clients at once. For each call to `async_write` the completion
        handler is called once when the data finishes sending, in a manner
        identical to `boost::asio::async_write`.

        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp#L1048) | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/connection.hpp#L672) |
        | --- | --- |
        | ```table-programlisting template<class ConstBufferSequence> void write(ConstBufferSequence const& buffers); ``` | ```table-programlisting lib::error_code send(std::string const & payload,     frame::opcode::value op = frame::opcode::text); ... lib::error_code send(message_ptr msg); ``` |
    ...
    :   websocketpp requires that the entire message fit into memory,
        ...
        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp#L1151) | websocketpp |
        | --- | --- |
        | ```table-programlisting template<class ConstBufferSequence> void write_some(bool fin,     ConstBufferSequence const& buffers); ``` | *<not available>* |
    ...
    :   The websocketpp read implementation continuously reads asynchronously
        ...
        an asynchronous initiation function (for example `beast::websocket::stream::async_read`) with an associated
        ...
        | Beast | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/connection.hpp#L728) |
        | --- | --- |
        | *<implicit>* | ```table-programlisting lib::error_code pause_reading(); lib::error_code resume_reading(); ``` |
    ...
    :   websocketpp offers the `endpoint`
        ...
        | [Beast](http://www.boost.org/doc/html/boost_asio/reference/async_connect.html), [also](http://www.boost.org/doc/html/boost_asio/reference/basic_socket_acceptor/async_accept.html) | [websocketpp](https://github.com/zaphoyd/websocketpp/blob/378437aecdcb1dfe62096ffd5d944bf1f640ccc3/websocketpp/transport/asio/endpoint.hpp#L52) |
        | --- | --- |
        | ```table-programlisting #include <boost/asio.hpp> ``` | ```table-programlisting template <typename config> class endpoint : public config::socket_type; ``` |
    ...
    :   Callers invoke `beast::websocket::accept`
        ...
        | [Beast](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp#L501), [also](https://github.com/vinniefalco/Beast/blob/6c8b4b2f8dde72b01507e4ac7fde4ffea57ebc99/include/beast/websocket/stream.hpp#L401) | websocketpp |
        | --- | --- |
        | ```table-programlisting template<class ConstBufferSequence> void accept(ConstBufferSequence const& buffers);  template<class Allocator> void accept(http::header<true, http::basic_fields<Allocator>> const& req); ``` | *<not available>* |