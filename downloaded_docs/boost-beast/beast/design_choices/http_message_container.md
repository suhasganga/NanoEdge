### [HTTP Message Container 🎦](http_message_container.html "HTTP Message Container 🎦")
...
used are available in the [GitHub
repository](https://github.com/vinniefalco/CppCon2017).
...
* `m` is an instance of an
  HTTP message container
* `x` is a series of octets
...
*start-line*, we would like the containers for requests
...
|  |  |
| --- | --- |
| ```table-programlisting /// An HTTP request template<class Fields, class Body> struct request {     int         version;     std::string method;     std::string target;     Fields      fields;      typename Body::value_type body; }; ``` | ```table-programlisting /// An HTTP response template<class Fields, class Body> struct response {     int         version;     int         status;     std::string reason;     Fields      fields;      typename Body::value_type body; }; ``` |

These containers are capable of representing everything in the model of HTTP
requests and responses described in [rfc7230](https://tools.ietf.org/html/rfc7230).
Request and response objects are different types. The user can choose the
container used to represent the fields. And the user can choose the **Body** type, which is a concept defining not only the
type of `body` member but also
...
specialization, or in C++17 a `constexpr`
expression:
...
and examine its contents before choosing a type for **Body**.
...
Code which accesses the fields has to laboriously mention the `fields` member, so we'll not only make
`header` a base class but we'll
...
of **Fields** there will need to be a set of
...
for the purposes of construction, except that instead of `first`
and `second` we have the `Fields` base class and `message::body`
member. This means that single-argument constructors for those fields should
be accessible as they are with `std::pair`,
and that a mechanism identical to the pair's use of `std::piecewise_construct`
...
support the scheme. It also means that request messages could have **four** different allocators: two for the fields and
...
to the **Fields** type. First, the interface
change:
...
string is delegated to the **Fields** container,
which can already be allocator aware and constructed with the necessary allocator
parameter via the provided constructor overloads for `message`.
...
`Body` far more often than
they will for `Fields`. Thus,
we swap the order of these types and provide a default. Then, we provide
type aliases for requests and responses to soften the impact of using `bool` to choose the specialization:
