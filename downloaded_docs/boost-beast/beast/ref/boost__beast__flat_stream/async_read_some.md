##### [flat\_stream::async\_read\_some](async_read_some.html "flat_stream::async_read_some")

Start an asynchronous read.

###### [Synopsis](async_read_some.html#beast.ref.boost__beast__flat_stream.async_read_some.synopsis)
...
###### [Description](async_read_some.html#beast.ref.boost__beast__flat_stream.async_read_some.description)
...
###### [Parameters](async_read_some.html#beast.ref.boost__beast__flat_stream.async_read_some.parameters)
...
| `handler` | The completion handler to invoke when the operation completes. The implementation takes ownership of the handler by performing a decay-copy. The equivalent function signature of the handler must be:   ```table-programlisting void handler(     error_code const & error,        // Result of operation.     std::size_t bytes_transferred   // Number of bytes read. ); ```   If the handler has an associated immediate executor, an immediate completion will be dispatched to it. Otherwise, the handler will not be invoked from within this function. Invocation of the handler will be performed by dispatching to the immediate executor. If no immediate executor is specified, this is equivalent to using `net::post`. |

###### [Remarks](async_read_some.html#beast.ref.boost__beast__flat_stream.async_read_some.remarks)
