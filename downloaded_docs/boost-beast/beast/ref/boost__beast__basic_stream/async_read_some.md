##### [basic\_stream::async\_read\_some](async_read_some.html "basic_stream::async_read_some")

Read some data asynchronously.

###### [Synopsis](async_read_some.html#beast.ref.boost__beast__basic_stream.async_read_some.synopsis)
...
###### [Description](async_read_some.html#beast.ref.boost__beast__basic_stream.async_read_some.description)
...
function. The program must ensure that no other calls to [`read_some`](read_some.html "basic_stream::read_some") or [`async_read_some`](async_read_some.html "basic_stream::async_read_some") are performed
...
###### [Parameters](async_read_some.html#beast.ref.boost__beast__basic_stream.async_read_some.parameters)
...
| `handler` | The completion handler to invoke when the operation completes. The implementation takes ownership of the handler by performing a decay-copy. The equivalent function signature of the handler must be:   ```table-programlisting void handler(     error_code error,               // Result of operation.     std::size_t bytes_transferred   // Number of bytes read. ); ```   If the handler has an associated immediate executor, an immediate completion will be dispatched to it. Otherwise, the handler will not be invoked from within this function. Invocation of the handler will be performed by dispatching to the immediate executor. If no immediate executor is specified, this is equivalent to using `net::post`. |

###### [Remarks](async_read_some.html#beast.ref.boost__beast__basic_stream.async_read_some.remarks)

The `async_read_some` operation
may not receive all of the requested number of bytes. Consider using the
function `net::async_read` if you need to ensure that
the requested amount of data is read before the asynchronous operation
completes.

###### [Per-Operation Cancellation](async_read_some.html#beast.ref.boost__beast__basic_stream.async_read_some.per_operation_cancellation)
...
if they are also supported by the socket's `async_read_some`
operation.